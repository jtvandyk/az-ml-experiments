...
def next_entry_number(text: str) -> int:
    """Highest existing `## N.` heading + 1. Returns 1 if none exist."""
...
def main() -> int:
    ...
    parser.add_argument("--title", required=True, help="Imperative-phrase title")
    parser.add_argument("--priority", choices=["high", "medium", "low"], default="medium")
    parser.add_argument("--problem", required=True, help="What's wrong, with a file/cell pointer")
    parser.add_argument("--fix", required=True, help="Concrete proposed approach")
    parser.add_argument("--blast-radius", required=True,
                        help="What files change; what could regress; how to verify")
    parser.add_argument("--backlog-path", type=Path, default=DEFAULT_PATH,
                        help=f"Backlog path (default: {DEFAULT_PATH})")
    ...
    if not path.exists():
        ...
        print(f"Created {path} with standard header.")
    ...
    # Ensure exactly one blank line between the existing trailing `---`
    # and the new entry.
