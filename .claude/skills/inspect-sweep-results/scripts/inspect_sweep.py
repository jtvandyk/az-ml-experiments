...
# Used to compute the "naive constant predictor" Brier baseline = p × (1−p).
PREVALENCE = {
...
# Hyperparameters worth clustering on (matches typical XGBoost sweep search space).
...
def metric_col(df, name):
    """Try metrics.<name>, then <name>; return Series of NaN if absent."""
    import pandas as pd
...
def summarise_outcome(df_all, outcome, *, top_n, ceiling):
    """Return a dict capturing all stats for one outcome — text formatting is downstream."""
    ...
    metrics = {
        m: summarise_metric(df, m)
        for m in ("val_auprc", "test_auprc", "train_auprc",
                  "val_brier", "test_brier", "val_precision_at_20", "best_iteration")
    }

    # Top-N trials by val_auprc.
    ...
    # Diagnostic checks on the best trial.
    ...
    if best:
        ...
        if vb is not None and prev is not None:
            ...
            diagnostics.append(("val_brier vs. prevalence baseline",
                                f"{fmt(vb)} vs {fmt(baseline)} (prev={prev:.1%}) — {verdict}"))
        elif prev is None:
            diagnostics.append(("val_brier vs. prevalence baseline",
                                f"prevalence unknown for {outcome!r}; add to PREVALENCE dict"))

        # 3. best_iteration ceiling check.
        if bi is not None:
            try:
                bi_v = float(bi)
                pct = bi_v / ceiling if ceiling else 0
                verdict = ("early stopping fired ✓" if pct < 0.8 else
                           "EARLY STOPPING NOT FIRING — raise n_estimators or reduce early_stopping_rounds")
        ...
        if not (tr != tr or v != v):
            ...
            label = ("minimal overfit" if gap <= 0.10 else
                     ...
                     "SERIOUS OVERFIT — raise reg_alpha / gamma / min_child_weight")
            diagnostics.append(("train→val gap (best)", f"{fmt(tr)} → {fmt(v)} (Δ={gap:+.3f}) — {label}"))

    # Hyperparameter clustering on top-N.
    clustering = {}
    for p in TUNABLES:
...
def render_markdown(result: dict) -> str:
    """Render one outcome's summary as Markdown."""
    if result["n_runs"] == 0:
    ...
    if result["clustering"]:
        ...
        for p, c in result["clustering"].items():
            lines.append(f"- `{p}`: min={c['min']}, median={c['median']}, max={c['max']} "
                         f"→ tighten next round to roughly [{c['min']}, {c['max']}]")
...
def main() -> int:
    ...
    parser.add_argument("--experiment", default="instability_xgboost",
                        help="MLflow experiment name (default: instability_xgboost)")
    parser.add_argument("--tracking-uri", default=None,
                        help="MLflow tracking URI (else uses MLFLOW_TRACKING_URI env)")
    parser.add_argument("--outcome", default=None,
                        help="Restrict to one outcome (default: all distinct tags.outcome values)")
    parser.add_argument("--top-n", type=int, default=5)
    parser.add_argument("--n-estimators-ceiling", type=int, default=300,
                        help="For the best_iteration saturation check (default 300)")
    parser.add_argument("--include-failed", action="store_true",
                        help="Include failed runs (default: skip)")
    ...
    if df.empty:
        print(f"No runs found in experiment {args.experiment!r}. "
              f"Verify the name (`mlflow experiments search`) and tracking URI.", file=sys.stderr)
        return 2
    ...
    if "tags.outcome" not in df.columns:
        print("warning: runs are not tagged with `outcome` — cannot group. "
              "Treating all runs as one outcome named '(untagged)'.", file=sys.stderr)
        df["tags.outcome"] = "(untagged)"

    outcomes = [args.outcome] if args.outcome else sorted(df["tags.outcome"].dropna().unique().tolist())

    results = [summarise_outcome(df, o, top_n=args.top_n, ceiling=args.n_estimators_ceiling)
               for o in outcomes]

    if args.json:
        # Strip non-serializable bits (Series, etc.).
        def clean(v):
