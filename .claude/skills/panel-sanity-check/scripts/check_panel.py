...
# Severity tags (kept ANSI-free so output stays grep-able).
ERROR = "ERROR"
WARN  = "WARN"
INFO  = "INFO"

DEFAULT_OUTCOME_COLS = [
    # Core (notebooks 01–13 sources)
    "civil_war_onset", "coup_attempt", "regime_backsliding",
    "mass_unrest_onset", "humanitarian_crisis_onset",
    # Supplementary (notebooks 14–18 sources)
...
def load_parquet(path: str):
    """Load parquet from a local path or abfss:// URL.

    abfss reads use DefaultAzureCredential — imported lazily so local-only
    runs don't need the Azure stack.
    """
    import pandas as pd  # local import — script-as-helper convention
    ...
    if path.startswith("abfss://"):
        try:
            ...
            raise
        # adlfs is the fsspec implementation pandas/pyarrow use under the hood
        try:
            import adlfs  # noqa: F401
        except ImportError as e:
            print(f"error: ADLS read requires adlfs: {e}", file=sys.stderr)
            raise
        # Extract account name from URL: abfss://container@<account>.dfs.core.windows.net/...
        import re
...
def check_country_year_coverage(df, findings: list, *, c_min: int, c_max: int,
    ...
    if years_expected is not None:
        ...
        if y_min != ey_min or y_max != ey_max:
            emit(findings, WARN,
                 f"year range {y_min}–{y_max} differs from expected {ey_min}–{ey_max}")
    ...
    emit(findings, INFO,
         f"rows={n_total:,}, full grid would be {expected_rows:,} "
         f"({coverage:.1%} coverage)")

    # Sparse countries / years.
    by_country = df.groupby("iso3")["year"].nunique()
    sparse_countries = by_country[by_country < 5].index.tolist()
    if sparse_countries:
        sample = sparse_countries[:10]
        emit(findings, WARN,
             f"{len(sparse_countries)} country/countries with <5 years of data: "
             f"{sample}{'...' if len(sparse_countries) > 10 else ''}")

    by_year = df.groupby("year")["iso3"].nunique()
    sparse_years = by_year[by_year < 40].index.tolist()
    if sparse_years:
        emit(findings, WARN,
             f"{len(sparse_years)} year(s) with <40 countries: {sparse_years[:10]}")
...
def check_outcome_labels(df, findings: list, outcome_cols: list[str]) -> None:
    ...
    for col in present:
        ...
        emit(findings, INFO,
             f"{col}: positives={n_pos:,}/{n_total:,} ({rate:.2%})")
...
def check_features(df, findings: list, *, missingness_threshold: float,
    ...
    if not high_miss.empty:
        ...
        emit(findings, WARN,
             f"{len(high_miss)} columns >{missingness_threshold:.0%} missing — "
             f"top entries: {[(c, f'{v:.0%}') for c, v in sample.items()]}")

    # Top-missing summary (informational).
    top = miss_frac.sort_values(ascending=False).head(top_missing)
    emit(findings, INFO,
         f"top-{top_missing} most-missing features: "
         f"{[(c, f'{v:.0%}') for c, v in top.items() if v > 0]}")
    ...
    if not numeric.empty:
        ...
        if nzv:
            ...
            emit(findings, level,
                 f"{len(nzv)} features with std<{nzv_threshold} ({nzv_pct:.1%} of features)")
            emit(findings, INFO, f"NZV columns: {nzv[:15]}{'...' if len(nzv) > 15 else ''}")

    # Silent-merge collision indicators.
    suspicious = [c for c in feature_cols if c.endswith("_x") or c.endswith("_y")]
    if suspicious:
        emit(findings, WARN,
             f"{len(suspicious)} columns end with _x / _y — likely a silent merge collision: "
             f"{suspicious[:10]}")
...
def report(findings: list[tuple[str, str]]) -> bool:
    """Print findings grouped by severity. Return True if any ERROR present."""
...
def main() -> int:
    ...
    parser.add_argument("--years-expected", type=parse_year_range, default=None,
                        help="Expected year range as 'YYYY-YYYY' (e.g. 2000-2024)")
    parser.add_argument("--outcome-cols",
                        default=",".join(DEFAULT_OUTCOME_COLS),
                        help="Comma-separated outcome columns required to be present")
    ...
    check_country_year_coverage(
        df, findings,
        c_min=args.countries_min, c_max=args.countries_max,
        years_expected=args.years_expected,
    )
