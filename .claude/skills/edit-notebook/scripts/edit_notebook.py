...
def apply_edit(cell: dict, edit: dict) -> str:
    """Mutate cell['source'] in place; return a one-line description for the log."""
    cid = edit["cell_id"]
    keys = {k for k in ("new_source", "old_substring", "append") if k in edit}
    if len(keys) != 1:
        raise ValueError(
            f"edit for {cid} must specify exactly one of new_source / old_substring / append; "
            f"got {sorted(keys)}"
        )
...
def main() -> int:
    ...
    parser.add_argument("--check-only", action="store_true",
                        help="Validate the spec without writing")
