...
Used by the `edit-notebook` skill to verify cell IDs before applying edits.
"""
import argparse
import json
import sys
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Print one line per notebook cell (index, type, id, source preview).",
    )
    parser.add_argument("notebook", type=Path, help="Path to .ipynb file")
    parser.add_argument(
        "--preview", type=int, default=90,
        help="Number of source characters to preview (default: 90)",
    )
